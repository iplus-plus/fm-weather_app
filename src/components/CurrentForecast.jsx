...
const MainInfo = ({ geoData, forecastData }) => {
    ...
    return (
        <div
            ...
            className="mt-8 flex h-[350px] flex-col items-center justify-center rounded-xl bg-cover bg-no-repeat p-4"
        >
...
const ExtraInfo = ({ forecastData }) => {
    ...
    const items = [
        { name: "feels like", value: current?.apparent_temperature, units: current_units?.apparent_temperature },
        { name: "humidity", value: current?.relative_humidity_2m, units: current_units?.relative_humidity_2m },
        { name: "wind", value: current?.wind_speed_10m, units: current_units?.wind_speed_10m },
        { name: "precipitation", value: current?.precipitation, units: current_units?.precipitation }
    ];
            ...
            {items.map(item => (
                ...
                >
                    ...
                    <p className="mt-4 text-3xl font-light">
                        {item.value} <span className="text-2xl opacity-20">{item.units}</span>
                    </p>
                </div>
            ))}
        </div>
